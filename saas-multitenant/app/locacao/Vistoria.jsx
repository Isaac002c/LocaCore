...
export function VistoriaFields({ value = EMPTY_VISTORIA, onChange }) {
  ...
  return (
    <div style={{ border: '1px solid #e2e8f0', borderRadius: 10, padding: 12, background: '#f8fafc' }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.3 }}>Vistoria</div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, margin: '6px 0 10px' }}>
        {ACCESSORIES.map(([key, l]) => (
          <label key={key} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, color: '#334155', cursor: 'pointer' }}>
...
export function VistoriaView({ data, title = 'Vistoria' }) {
  ...
  return (
    <div style={{ fontSize: 13, color: '#334155', lineHeight: 1.6 }}>
      <div style={{ fontWeight: 700, color: '#0f172a', marginBottom: 2 }}>{title}</div>
      ...
      {d.notes && <div style={{ color: '#64748b', fontStyle: 'italic' }}>{d.notes}</div>}
    </div>
  );
}
